...
## ✨ 特徴
- **フロントエンド完結（サーバーレス）**: HTML/CSS/JS のみで動作し、GitHub Pages や各種静的ホスティング環境で簡単に公開可能。
...
3. **音声ファイルの選択**:
   - 「音声ファイルを選択」から、録音した講義音声（短尺推奨）を選択。
   - 選択後、プレビュープレイヤーで再生確認が可能。
4. **実行**:
   - 「✨ 文字起こし＆要約を実行」ボタンをタップ（処理には数十秒かかります）。
5. **結果の確認・コピー**:
   - 「📊 要約 & 要点」タブと「📝 全文文字起こし」タブを切り替えて結果を確認。
   - 各エリアのコピーボタンでクリップボードへ一括取得が可能。

## 🛠️ 技術スタック
- **Frontend**: HTML5, Vanilla JavaScript, Tailwind CSS (CDN)
- **AI Model**: Google Gemini API (`gemini-3.5-flash-lite`)
- **Hosting**: GitHub Pages 等
